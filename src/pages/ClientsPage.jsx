...
function ClientsPage() {
  ...
  const FilteredData = clients
    .sort((a, b) => {
      if (selectMonth !== "all") {
        // Ordena por aniversário se o filtro de mês não for "all"
        const aBirthday = new Date(a.date);
        const bBirthday = new Date(b.date);

        // Adiciona 1 dia aos objetos Date
        aBirthday.setDate(aBirthday.getDate() + 1);
        bBirthday.setDate(bBirthday.getDate() + 1);

        // Calcula o valor de comparação usando apenas mês e dia
        const aCompareValue = aBirthday.getMonth() * 100 + aBirthday.getDate();
        const bCompareValue = bBirthday.getMonth() * 100 + bBirthday.getDate();

        return aCompareValue - bCompareValue;
      } else {
        // Ordena por nome se o filtro de mês for "all"
    ...
    .filter((clientItem) => {
      ...
      if (selectTime === "never") {
        if (consultationsClient.length === 0) {
          console.log(consultationsClient.length);
          return clientItem;
        }
      }
            ...
            <thead className="">
              <tr className="text-md font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
                ...
                <th className="px-4 py-3 text-center">
                  {selectMonth !== "all" ? "Aniversário" : "Data de Nascimento"}
                </th>
